...
n_sources = 16 # the files include at least n_sources step response file + one validation file
source_labels = ['pilot soc','ic_pm8009','ic_pm855','ic_pm855L','pm855b',\
                 'ic_da9313','main cam mada','main cam sensor','mcp',\
                     'pilot_dram','pilot_pmic','soc','top_ic','tp panel',\
                         'wide_cam_mada','wide_cam_sensor']
yDATA_series = RC_data_import.y_traces_import(path_to_files,filenames_y,'csv',cleanup_flag)
# uDATA_series = RC_data_import.u_traces_import(path_to_files,filenames_u,'csv',cleanup_flag)
...
# [time_data_pwr, u_data, u_labels] = uDATA_series[0]
# pwr_valid3 = u_data.ravel()


### ===================== Thermal RC network from impedance curves ============ 
import RC_imped
## power of each active source for step response with one active source
pwr_train = [7,1,1,1,1,1,1,7,1,1,1,7,1,7,1,1]
...
## power of each active source in one scenario
## FindX2ProCC 4K30UD (900s), 4K30SN (2000s) and 4K60 (900s) 
pwr_valid = [[0.7625,0.049,0.219,0.33,0.094,0.115,0.221,0.48,0.096,0.142,0.1839,2.226,0.02,0.303,0.04,0.006],
             [0.8583,0.049,0.219,0.33,0.094,0.115,0.221,0.48,0.096,0.1549,0.2191,2.226,0.02,0.303,0.04,0.006],
            [0.5939,0.059,0.324,0.303,0.104,0.43,0.216,0.562,0.135,0.1608,0.1723,2.855,0.02,0.749,0.043,0]] 
...
# my_delim = ","
# my_header = source_labels + ['Total_power(W)','Label_Ts_ToT_max','Ts_ToT_max',\
...
for idx_sel_sources in [11]:
    ...
    case="Temperature traces of step power (1W) on " + source_labels[idx_sel_sources]
